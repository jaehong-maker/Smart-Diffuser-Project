...
// 함수 원형 (기존 그대로)
void bootAnimation(); void handleInput(String input); void stopSystem(); void printMainMenu();
void sendServerRequest(String payload); void printCalibrationInfo(); void runManualMode(String input);
void forceAllOff(); void manageWiFi(); void systemHeartbeat(); void handleWebClient();
void runSprayLogic(); void checkSafety(); void runAutoDemoLoop(); void redrawInputLine(String &buffer);
void checkSerialInput(); void connectWiFi(); void printDashboard(); void playSound(int trackNum);
void autoWeatherScheduler(); void changeVolume(int vol); void initMicrophone(); 
int32_t readMicrophone(); void pollServer(); void recordAndSendVoice(); void monitorWeight(); 
void updateDisplay(int iconID, String text); void updateProgressBar(int val); 
void runSoundVisualizer(); void runSoundReaction();   
void checkNextionInput(); void handleNextionCmd(const String &cmd); 
void enterWeatherMode(bool runNow); void showPrompt(); 
void nexSend(const String &cmd); void nexSetText(const char* obj, String text);
void nexSetVal(const char* obj, int v); void nexSetPic(const char* obj, int picId);
...
void setup() {
  ...
  scale1.begin(LC1_DT, LC1_SCK); scale2.begin(LC2_DT, LC2_SCK);
  scale3.begin(LC3_DT, LC3_SCK); scale4.begin(LC4_DT, LC4_SCK);
...
void nexSetText(const char* obj, String text) { text.replace("\"", "'"); nexSend(String(obj) + ".txt=\"" + text + "\""); }
void nexSetVal(const char* obj, int v) { nexSend(String(obj) + ".val=" + String(v)); }
void nexSetPic(const char* obj, int picId) { nexSend(String(obj) + ".pic=" + String(picId)); }
void updateDisplay(int iconID, String text) { nexSetPic("p0", iconID); nexSetText("t0", text); }
void updateProgressBar(int val) { nexSetVal("j0", constrain(val, 0, 100)); }
...
void checkSerialInput() {
  ...
  if (currentMode == 4) {
    if (Serial.available() > 0) {
      ...
      if (c == '+') { calibration_factor += 10; scale1.set_scale(calibration_factor); printCalibrationInfo(); }
      else if (c == '-') { calibration_factor -= 10; scale1.set_scale(calibration_factor); printCalibrationInfo(); }
      else if (c == 't') { scale1.tare(); Serial.println("영점"); printCalibrationInfo(); }
...
void handleInput(String input) {
  if (currentMode == 0) {
    ...
    else if (input == "4") { currentMode = 4; Serial.println(C_YELLOW "\r\n--- [ Setting ] ---" C_RESET); }
...
void runSoundReaction() {
    ...
    if (bytes_read > 0) {
        ...
        for (int i = 0; i < numSamples; i++) {
            int16_t wave = (samples[i] >> 14); sumSquares += (double)wave * wave;
        }
...
void runSprayLogic() { if (activePin == -1) return; if (isSpraying) { if (millis() - prevMotorMillis >= sprayDuration) { digitalWrite(activePin, HIGH); isSpraying = false; prevMotorMillis = millis(); if (currentMode == 1) { stopSystem(); showPrompt(); return; } Serial.print("\r\033[K"); Serial.printf(C_CYAN "      └── [Idle] 휴식...\r\n" C_RESET); showPrompt(); Serial.print(inputBuffer); updateDisplay(0, "Resting..."); } } else { if (millis() - prevMotorMillis >= REST_TIME) { forceAllOff(); digitalWrite(activePin, LOW); isSpraying = true; prevMotorMillis = millis(); Serial.print("\r\033[K"); Serial.printf(C_GREEN "      ┌── [Action] 재분사!\r\n" C_RESET); showPrompt(); Serial.print(inputBuffer); int icon = 0; if (activePin == PIN_SUNNY) icon=1; else if (activePin == PIN_CLOUDY) icon=2; else if (activePin == PIN_RAIN) icon=3; else if (activePin == PIN_SNOW) icon=4; updateDisplay(icon, "Spraying..."); } } }
...
void printCalibrationInfo() { Serial.printf("📡 보정값: %.1f | 현재 무게: %.2f g\r\n", calibration_factor, scale1.get_units(5)); }
// ★ 로드셀 4개 모니터링 함수 수정
void monitorWeight() { 
    if (millis() - lastWeightCheckTime > 500) { 
        lastWeightCheckTime = millis(); 
        weights[0] = scale1.get_units(2); weights[1] = scale2.get_units(2);
        weights[2] = scale3.get_units(2); weights[3] = scale4.get_units(2);
