 ...
 * [버  전] V10.9.1 Echo Fix (Final Polish)
 * [수  정] ESP32가 보낸 명령어가 되돌아오는 '메아리 현상' 무시 기능 추가
 */
...
// 4. Nextion (팀원 설정: 5, 17)
const int NEXTION_TX_PIN = 5;   
const int NEXTION_RX_PIN = 17;  
...
void setup() {
  ...
  Serial.printf(C_BOLD    " 🚀 SMART DIFFUSER V10.9.1 (Clean) \r\n" C_RESET);
  Serial.printf(C_MAGENTA "****************************************\r\n" C_RESET);
...
void checkSerialInput() {
  ...
  if (currentMode == 4) {
    if (Serial.available() > 0) {
      ...
      else if (c == 't') { scale.tare(); Serial.println("영점"); printCalibrationInfo(); }
