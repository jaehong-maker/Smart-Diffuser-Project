...
export function LedSettingsScreen() {
  ...
  const { isLedOn, setIsLedOn, ledColor, setLedColor, ledBrightness, setLedBrightness, sendDeviceData } = useDevice();
  ...
  useEffect(() => {
    // 로컬 상태가 변경되었을 때, 사용자가 직접 변경한 경우(isInteracting)에만 서버로 전송
    if (isInteractingRef.current) {
      const rgb = hexToRgb(ledColor);
      syncWithHardware(rgb.r, rgb.g, rgb.b, ledBrightness, isLedOn, effect);
    }
  }, [ledColor, ledBrightness, isLedOn, effect]);
    ...
    <div key="led-screen" className="flex-1 flex flex-col bg-white dark:bg-gray-950 overflow-x-hidden relative pb-32 min-h-screen transition-colors duration-500">
      <div className="absolute top-0 left-0 right-0 h-[45vh] pointer-events-none overflow-hidden">
        <motion.div 
          animate={{ 
            background: isLedOn 
              ? `radial-gradient(circle at 50% 40%, ${ledColor}33 0%, transparent 70%)` 
      ...
      <div className="px-6 py-4 flex flex-col gap-6 z-10 relative">
        <div className="flex flex-col items-center justify-center py-10 relative min-h-[280px]">
            ...
            <AnimatePresence>
              {isLedOn && (
                <motion.div
                  ...
                  style={{ 
                    background: `radial-gradient(circle, ${ledColor}4d 0%, transparent 75%)`,
                    filter: 'blur(10px)'
                  }}
                />
              )}
          ...
          <motion.div 
            ...
            whileTap={{ scale: 0.95 }}
            onClick={() => { markInteraction(); setIsLedOn(!isLedOn); }}
            ...
            style={isLedOn ? { 
              ...
              boxShadow: `0 15px 45px ${ledColor}66, inset 0 0 20px rgba(255,255,255,0.4)`
            } : {
              boxShadow: "0 10px 30px rgba(0,0,0,0.02), inset 0 2px 5px rgba(255,255,255,0.9)"
            }}
          >
            <Lightbulb className={`w-10 h-10 transition-all duration-700 ${isLedOn ? "text-white drop-shadow-[0_0_10px_rgba(255,255,255,0.8)]" : "text-gray-300"}`} />
          </motion.div>
          ...
          <div className="mt-12 flex bg-gray-100/80 dark:bg-gray-900/80 backdrop-blur-md p-1 rounded-2xl w-40 shadow-inner z-10 relative border border-gray-200/50 dark:border-gray-800/50">
            <button
              onClick={() => { markInteraction(); setIsLedOn(true); }}
            ...
            <button
              onClick={() => { markInteraction(); setIsLedOn(false); }}
              className={`flex-1 py-2.5 rounded-xl text-[10px] font-black tracking-widest transition-all duration-300 ${
                !isLedOn 
                  ? "bg-white dark:bg-gray-800 text-gray-900 dark:text-white shadow-md" 
                  : "text-gray-400 hover:text-gray-500"
        ...
        <div className="grid grid-cols-1 gap-6">
          <div className="bg-white/70 dark:bg-gray-900/70 backdrop-blur-xl rounded-[2.5rem] p-8 shadow-xl border border-white/40 dark:border-gray-800/40 transition-all duration-500">
            ...
            <ColorWheel 
              ...
              onChange={(newColor) => { markInteraction(); setLedColor(newColor); }}
              disabled={!isLedOn || effect === 2} // 무지개 모드일 때는 컬러휠 비활성화
          ...
          <div className="bg-white/70 dark:bg-gray-900/70 backdrop-blur-xl rounded-[2rem] p-6 shadow-lg border border-white/40 dark:border-gray-800/40">
            <div className="flex items-center justify-between mb-6">
              ...
              <span className="text-sm font-black text-gray-900 dark:text-white">{ledBrightness}%</span>
            </div>
            ...
            <div className="px-2">
              <div className="relative w-full h-1.5 bg-gray-200 dark:bg-gray-800 rounded-full overflow-hidden">
                <motion.div 
                  initial={false}
                  animate={{ width: `${ledBrightness}%`, backgroundColor: isLedOn ? ledColor : "#9ca3af" }}
                ...
                <input
                  ...
                  onChange={(e) => { markInteraction(); setLedBrightness(Number(e.target.value)); }}
                  disabled={!isLedOn}
