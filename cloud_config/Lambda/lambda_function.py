...
def _parse_spray_code(raw_spray):
    if isinstance(raw_spray, int): return raw_spray
    val = str(raw_spray)
    numbers = re.sub(r'[^0-9]', '', val) # 숫자가 아닌 모든 문자 제거
...
def _build_ambient_response(scent_code, duration, context_key, db_level, result_text, scores=None):
    payload = {
        "cmd": int(scent_code),
        "dur": int(duration),
        "music": 0,
   ...
   # [ACTION] SEND_AUTH: 인증번호 이메일 발송
    ...
    # [ACTION] LOGIN
        ...
        try:
            ...
            if 'Item' in res:
                ...
                try:
                    db_utils.state_table.update_item(Key={'deviceId': my_device}, UpdateExpression="set music_tracks = :m", ExpressionAttributeValues={':m': str(my_music)})
                except: pass
    ...
    # [ACTION] FEEDBACK
        ...
        try:
            ...
            # 앱은 "향기번호_상황" 문자열을 'data' 키로 보냄
            data_str = body.get("data", "1_Unknown")
            parts = data_str.split("_", 1)

            scent = int(parts[0]) if parts[0].isdigit() else 1
            ...
            db_utils.save_feedback(device_id, ctx, scent, weight_val)
            ...
            if not is_like:
                ...
                # ★ [4] 새로 추천된 향기에도 가중치 +1 부여 (사용자 유도)
                db_utils.save_feedback(device_id, ctx, new_scent, 1)
                ...
                return {
                    ...
                    "headers": {"Content-Type": "application/json"},
                    "body": json.dumps({
                        "message": f"싫어요 반영! {new_scent}번 향으로 우회 학습",
                        "spray": new_scent,
    ...
    if action == "POLL":
        ...
        logger.info(f"[POLL_DATA] 수신된 전체 무게 배열: {body.get('weights')} | 정규화 배열: {normalized_weights}")

        try:
            db_utils.state_table.update_item(
                Key={'deviceId': device_id},
                UpdateExpression="set weight_g = :wg, weights = :w",
        ...
        if auto_stop_str:
            try:
                ...
                if now_kst >= auto_stop_time:
                    # 약속된 시간이 지났으면 우체통에 정지명령(0)을 넣고 타이머 삭제
    ...
    # [ACTION] MANUAL
        ...
        try:
            # "spray12"와 같은 문자가 와도 12로 추출 (혼합 향기 에러 방지)
            raw_spray = body.get("spray", 1)
            spray_code = _parse_spray_code(raw_spray)
            ...
            db_utils.log_table.put_item(Item={
                ...
                "duration": int(duration),
                "region": region_req or "Manual",
                "weight_g": safe_dec(normalized_weights[0] if normalized_weights else 0),
                "weights": [safe_dec(weight) for weight in normalized_weights] if normalized_weights else [safe_dec(0)]*4,
                "temp": safe_dec(0),
                "humid": 0,
                "feedback": 1
            })
        ...
        try:
            now_kst = _get_kst_now()
            db_level_raw = body.get("db_level", body.get("noise_level", body.get("noise", 0)))
            context_key, time_slot, noise_bucket, normalized_db = _build_ambient_context(db_level_raw, now_kst)

            db_utils.seed_ambient_defaults()
            spray_code, ambient_scores = db_utils.get_ambient_recommendation(context_key)
            duration = _parse_int(
...
# [A] 감성 모드
    if mode == "emotion":
        ...
        # 에러 나던 gpt_recommendation 대신 gemini로 교체
        gpt_spray_str, gpt_reason = api_utils.ask_gemini_recommendation(f"현재 내 기분은 {user_emo} 상태야.", "취향 데이터 없음")
        gpt_spray = _parse_spray_code(gpt_spray_str) # 숫자만 안전하게 추출
        
        if gpt_spray > 0:
            spray_code, result_text, logic_name = gpt_spray, gpt_reason, "AI_Gemini_Emotion"
        else:
        ...
        try: # ★ 날씨 로직 전체를 try로 감싸기 시작
            if region == "테스트맑음":
                ...
                if "SERVICE_KEY" in os.environ:
                    ...
                    # ★ 방어 코드: 기상청 API가 죽었거나 날씨 값이 비어있을 때
                    if not w_res:
                        w_res = "날씨 알 수 없음(API 지연)"
                        temp, humidity = "20", "50"
